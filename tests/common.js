...
(() => {
  ...
  function openEditor(item,data){if(!isAdmin())return;document.getElementById("cm-editor")?.remove();const content=el("select",{},["campaign","merchant_offer","social_post","awareness","review"].map(v=>el("option",{value:v,selected:item.content_type===v},t(v==="review"?"review_type":v)))),category=el("select",{},data.categories.map(r=>el("option",{value:r.id,selected:(item.campaign_category||item.primary_category)===r.id},taxonomyName(r)))),title=el("input",{value:item.title||""}),summary=el("textarea",{rows:4},item.snippet||item.summary||""),status=el("input",{value:item.current_status||"",readonly:true,title:"Calculated automatically from campaign dates"}),active=el("input",{type:"checkbox",checked:item.active!==false,disabled:true,title:"Calculated automatically from campaign dates"}),pub=el("input",{type:"date",value:dateInput(item.published_at)}),start=el("input",{type:"date",value:dateInput(item.start_date)}),end=el("input",{type:"date",value:dateInput(item.end_date)}),official=el("input",{value:item.official_campaign_page_url||"",type:"url"}),primary=el("input",{value:item.primary_official_source_url||"",type:"url"}),mechanic=el("textarea",{rows:2},item.mechanic||""),eligibility=el("textarea",{rows:2},item.eligibility||""),terms=el("textarea",{rows:3},item.terms_note||"");const social=Object.fromEntries(["instagram","x","facebook","tiktok"].map(p=>[p,el("input",{value:item.social_links?.[p]||"",type:"url"})]));const linkSelect=el("select",{},el("option",{value:""},t("noCampaign")),...campaignsFor(item,data).map(c=>el("option",{value:c.id,selected:item.campaign_id===c.id||item.suggested_campaign_id===c.id},c.title||c.id)));
    ...
    const modal=el("div",{id:"cm-editor",class:"modal-backdrop"},el("section",{class:"modal"},el("header",{class:"modal__header"},el("h2",{},t("editItem")),el("button",{class:"icon-button",onclick:()=>modal.remove()},"×")),el("div",{class:"modal__body"},el("p",{class:"editor-note"},t("editsNote")),el("div",{class:"editor-grid"},field(t("contentType"),content),field(t("category"),category),field(t("title"),title),field(t("currentStatus"),status),field(t("summary"),summary),field(t("active"),active),field(t("published"),pub),field(t("startDate"),start),field(t("endDate"),end),field(t("officialCampaignUrl"),official),field(t("primarySourceUrl"),primary),field(t("mechanic"),mechanic),field(t("eligibility"),eligibility),field(t("terms"),terms),item.source_type==="social"?field(t("linkToCampaign"),linkSelect):null,field(t("instagramUrl"),social.instagram),field(t("xUrl"),social.x),field(t("facebookUrl"),social.facebook),field(t("tiktokUrl"),social.tiktok))),el("footer",{class:"modal__footer"},createFromPost,el("button",{class:"button button--ghost",onclick:()=>{resetItemOverride(item.id);location.reload();}},t("resetEdit")),el("button",{class:"button button--secondary",onclick:exportOverrides},t("exportEdits")),item.content_type==="campaign"?el("button",{class:"button button--danger",onclick:()=>deleteCampaign(item)},t("deleteCampaign")):null,el("button",{class:"button button--primary",onclick:()=>{const cat=category.value,ctype=cat==="merchant"?"merchant_offer":content.value,links=Object.fromEntries(Object.entries(social).map(([k,input])=>[k,input.value.trim()]).filter(([,v])=>v));const nextStart=toIsoDate(start.value),nextEnd=toIsoDate(end.value),life=lifecycleFor({start_date:nextStart,end_date:nextEnd}),nextTerms=(nextEnd&&staleNoEndNote(terms.value))?"":terms.value.trim();saveItemOverride(item.id,{title:title.value.trim(),snippet:summary.value.trim(),summary:summary.value.trim(),content_type:ctype,campaign_category:cat,current_status:life.status,active:life.active,published_at:toIsoDate(pub.value),start_date:nextStart,end_date:nextEnd,official_campaign_page_url:official.value.trim(),primary_official_source_url:primary.value.trim(),link:official.value.trim()||primary.value.trim()||item.link,mechanic:mechanic.value.trim(),eligibility:eligibility.value.trim(),terms_note:nextTerms,social_links:links,linked_campaign_id:linkSelect.value||null,campaign_id:linkSelect.value||item.campaign_id||null,review_required:ctype==="review"||(item.source_type==="social"&&!linkSelect.value&&item.review_required),review_reasons:ctype==="review"?["manual_review_required"]:[]});location.reload();}},t("saveLocal")))));document.body.appendChild(modal);modal.addEventListener("click",e=>{if(e.target===modal)modal.remove();});}
