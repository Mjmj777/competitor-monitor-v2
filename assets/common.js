...
(() => {
  ...
  Object.assign(I18N.ar,{
    ...
    campaignChanges:"تغيّرات الحملات",
    campaignChanges30d:"تغيّرات الحملات",
    campaignChangesNote:"أحداث سوقية موثقة خلال آخر {days} يومًا؛ لا تشمل المراجعات البشرية أو استكمال البيانات الناقصة.",
  ...
  Object.assign(I18N.en,{
    ...
    campaignChanges30d:"Campaign changes",
    campaignChangesNote:"Verified market events during the last {days} days; Admin reviews and data backfills are excluded.",
  ...
  Object.assign(I18N.ar,{
    exploreCompetitors:"استكشف المنافسين",exploreCompetitorsHint:"اختر منافسًا لعرض حملاته وتحليلاته في صفحته المستقلة.",
    customPeriod:"فترة مخصصة",fromDate:"من",toDate:"إلى",applyPeriod:"تطبيق",
    ...
    siteExperience:"شكل الموقع",siteExperienceTitle:"تجربة واجهة الموقع",siteExperienceHint:"عاين التصميم عندك أولًا، ثم انشره لجميع المستخدمين عندما تتأكد.",classicView:"التصميم الكلاسيكي",classicViewHint:"واجهة الموقع الحالية المألوفة.",intelligenceOsView:"Intelligence OS — داكن",intelligenceOsViewHint:"واجهة داخلية داكنة تركز على البيانات والرسوم والملخص الإداري.",previewLayout:"معاينة عندي",publishLayout:"نشر للجميع",publishedLayout:"التصميم المنشور",previewingLayout:"وضع المعاينة",layoutPublished:"تم نشر تصميم الموقع بنجاح.",layoutQueued:"جاري حفظ تصميم الموقع…",marketOverview:"نظرة عامة على السوق",marketOverviewDetail:"مخزون الحملات الموثقة ونشاط المنافسين."
  });
  Object.assign(I18N.en,{
    ...
    customPeriod:"Custom period",fromDate:"From",toDate:"To",applyPeriod:"Apply",
    ...
    siteExperience:"Site Experience",siteExperienceTitle:"Site Experience",siteExperienceHint:"Preview a layout privately first, then publish it for every user when ready.",classicView:"Classic View",classicViewHint:"The familiar current interface.",intelligenceOsView:"Intelligence OS — Dark",intelligenceOsViewHint:"An internal dark interface focused on data, charts and the management summary.",previewLayout:"Preview for me",publishLayout:"Publish for everyone",publishedLayout:"Published layout",previewingLayout:"Preview mode",layoutPublished:"The site layout was published successfully.",layoutQueued:"Saving the site layout…",marketOverview:"Market Overview",marketOverviewDetail:"Verified campaign inventory and competitor activity."
  });
  ...
  const competitorLogo=id=>{const safe=String(id||"").replace(/[^a-z0-9-]/gi,"");return document.body.classList.contains("layout-intelligence-os")&&["barq","tiqmo"].includes(safe)?`assets/logos/${safe}-dark.png`:`assets/logos/${safe}.webp`;};
  ...
  async function submitAdminDecision(payload,statusNode=null){
    ...
    try{
      ...
      const requestId=result.request_id,started=Date.now();
      while(Date.now()-started<30*60*1000){
        const check=await fetch(`/__review-status?request_id=${encodeURIComponent(requestId)}`,{cache:"no-store",credentials:"same-origin"});
        ...
        if(check.ok&&progress.status==="completed"){
          ...
          if(layoutAction)localStorage.removeItem("cm_home_layout_preview");
    ...
    }catch(error){if(statusNode)statusNode.textContent="";alert(`${t("reviewSaveFailed")}: ${error?.message||error}`);return false;}
  }
  function openMergeDialog(item,data){
    if(!isAdmin()||item.content_type!=="campaign")return;
    if(item.merged_into){if(confirm(t("undoMergeConfirm")))submitAdminDecision({action:"undo_merge",item_ids:[item.id]});return;}
    document.getElementById("cm-merge")?.remove();
    const protectedCategories=new Set(["remittance","musaned","sadad","card","engagement"]),sourceCategory=item.campaign_category||item.primary_category||"other";
    const targets=(data.items||[]).filter(row=>{const targetCategory=row.campaign_category||row.primary_category||"other",compatible=!(protectedCategories.has(sourceCategory)&&protectedCategories.has(targetCategory)&&sourceCategory!==targetCategory);return row.id!==item.id&&row.competitor_id===item.competitor_id&&row.content_type==="campaign"&&row.active!==false&&!row.merged_into&&compatible;});
    if(!targets.length)return alert(t("noMergeTargets"));
    const select=el("select",{},...targets.sort((a,b)=>String(a.title||"").localeCompare(String(b.title||""))).map(row=>el("option",{value:row.id},row.title||row.id)));
    const status=el("span",{class:"review-save-status-inline"});
    const modal=el("div",{id:"cm-merge",class:"modal-backdrop"},el("section",{class:"modal modal--compact"},el("header",{class:"modal__header"},el("h2",{},t("mergeCampaignTitle")),el("button",{class:"icon-button",onclick:()=>modal.remove()},"×")),el("div",{class:"modal__body"},el("p",{class:"editor-note"},t("mergeCampaignHint")),field(t("mergeInto"),select),status),el("footer",{class:"modal__footer"},el("button",{class:"button button--ghost",onclick:()=>modal.remove()},t("cancel")),el("button",{class:"button button--primary",onclick:event=>{if(!confirm(t("mergeConfirm")))return;event.currentTarget.disabled=true;submitAdminDecision({action:"merge_campaigns",item_ids:[item.id],target_campaign_id:select.value},status);}},t("mergeCampaign")))));
