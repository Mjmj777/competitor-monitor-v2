...
(() => {
  ...
  function openMergeDialog(item,data){
    ...
    const targets=(data.items||[]).filter(row=>row.id!==item.id&&row.competitor_id===item.competitor_id&&row.content_type==="campaign"&&row.active!==false&&!row.merged_into);
